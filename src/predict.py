...
def build_features_for_one_stock(price_df: pd.DataFrame, stock_row: pd.Series, active_features: list[str]) -> pd.DataFrame:
    ...
    df = df.dropna(subset=active_features + ["Close"]).reset_index(drop=True)
    return df
...
def main(top_n: int = 10):
    ...
    if not TRAIN_FEATURES_PATH.exists():
        raise FileNotFoundError(f"Missing {TRAIN_FEATURES_PATH}. Run model_train.py first.")
    ...
    for idx, (_, row) in enumerate(stock_df.iterrows(), start=1):
        ...
        feature_df = build_features_for_one_stock(full_df, row, active_features)
        if feature_df.empty:
        ...
        try:
            X = pd.DataFrame([latest[active_features]], columns=active_features)
